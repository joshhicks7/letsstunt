...
type Props = {
  ...
  /** When false, hide the large title (e.g. title is shown in the screen header). */
...
export function GroupDetailHero({
  ...
  return (
    <View style={styles.wrap}>
      <ThemedText style={[styles.badge, { color: colors.tint, marginBottom: showHeroTitle ? 0 : SPACING.xs }]}>
        Group
      </ThemedText>
      {showHeroTitle ? <ThemedText style={[styles.groupTitle, { color: colors.text }]}>{title}</ThemedText> : null}
      {locationCity ? (
