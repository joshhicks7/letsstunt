...
const MAX_MEDIA = 6;
const STEP_COUNT = 4;
const STEP_TITLES = ['About you', 'Your roles', 'Skills & experience', 'Photos'];
...
export default function OnboardingScreen() {
  ...
  const addPhoto = async () => {
    ...
    if (!r.canceled && r.assets[0]) {
      ...
      setDraft((prev) => ({
        ...prev,
        media: [...prev.media, { id: newId('m'), uri, type: 'image' as const }].slice(0, MAX_MEDIA),
      }));
    }
  };
  ...
  const renderStep3 = () => (
    <>
      <ThemedText style={[styles.cardStepMeta, { color: colors.secondary, marginBottom: SPACING.sm }]}>
        Optional — your first photo is the one partners see first. Add more anytime from your profile.
      </ThemedText>
      {draft.media.length > 0 ? (
        <View style={styles.onboardingPhotoRow}>
      ...
      {draft.media.length < MAX_MEDIA ? (
        <Pressable style={[styles.photoBtn, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={addPhoto}>
          <FontAwesome name="plus" size={18} color={colors.tint} />
          <ThemedText style={{ color: colors.text, marginLeft: SPACING.sm }}>Add photo</ThemedText>
        </Pressable>
      ) : null}
    </>
  );
...
const styles = StyleSheet.create({
  ...
  onboardingPhotoThumb: { width: '100%', height: '100%' },
  onboardingRemoveBtn: {
    position: 'absolute',
    top: 4,
    right: 4,
